...
function login_empresa(ventana_origen,mobile){
  ...
  //alert("LOGIN EMPRESA: "+mobile);
    ...
    mantencion=false;
	//document.loginFormEmpresa.tx_empresa.value = 'arcor';
    if(mantencion==false || document.loginFormEmpresa.tx_usuario.value=="soporte"){
...
function IniciaSessionAbierta(){
	if(window.localStorage.SESSION!=null){
		var Data = window.localStorage.SESSION;
		$.post('http://192.168.1.183/angular/autentificacion/cookies.php',
		{
			cookies_Data:Data,
		}, 
		function(data){
				if(data=='2'){
					...
					cookies=data1[6];
					usar_ssl = "N";
					//FALTA TRAER NUEVA COOKIE E INSERTAR
...
function loadurl_empresa(dest,mobile) {
  ...
  if(document.loginFormEmpresa.chk_guardasesion==undefined){
	chk_guardasesion = 0;
  }
  else{
    if(document.loginFormEmpresa.chk_guardasesion.checked==true){
	  chk_guardasesion = 1;
	}
	else{
	  chk_guardasesion = 0;
  ...
  //BUSCAR LA POSICION DEL USUARIO (SOLO MOVIL)
  //var latitud_gps = ""; 
  //var longitud_gps = "";;  
 
  //navigator.geolocation.getCurrentPosition(foundLocation1, noLocation1);

  latitud_gps = "0";
  longitud_gps = "0";
  
  xmlhttp.send("tx_empresa="+URLEncode(document.loginFormEmpresa.tx_empresa.value)+"&tx_usuario="+URLEncode(document.loginFormEmpresa.tx_usuario.value)+"&tx_contrasena="+URLEncode(document.loginFormEmpresa.tx_contrasena.value)+"&mobile="+URLEncode(mobile)+"&tx_phonegap="+URLEncode(document.loginFormEmpresa.tx_phonegap.value)+"&chk_guardasesion="+chk_guardasesion+"&latitud_gps="+latitud_gps+"&longitud_gps="+longitud_gps);

}

function foundLocation1(position){ 
...
function triggered_empresa() {
  ...
  if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200)) {
	...
	//GUARDAR EL TOKEN EN EL MOVIL (SOLO EN PHONEGAP)
	if(localStorage.getItem("url_servidor")!=null){
	  window.sessionStorage.setItem("token_sesion",token_sesion); 
	}
    ...
    if(data!="OK")
      ...
      //if(nueva=="si" || nueva=="si ") {
		...
		if(phonegap==false){
		  ...
		  //transition = "none";
	      $.mobile.changePage('http'+ssl+':'+doble_slash+host+'/principal/principal.php', { transition: transition} );
	  
		}
	    else if(phonegap==true){
		  //window.location='file:///android_asset/www/principal/principal.html';
		  $.mobile.changePage('file:///android_asset/www/principal/principal.html',{transition: transition});
		}
...
var strMsgError = '';	
	...
	else
	{
			...
			Loadreinicio('aplicacion/BusinessLogic/ConsultaExiste.php','Consulta');
	}
...
function Retornofn() {
  ...
  if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200)) {
	...
	if(data[0]==0){
		...
		CargandoCodigo();
		LoadreMail('aplicacion/BusinessLogic/ConsultaExiste.php','Mail',data[1]);
...
var strMsgError = '';
	...
	{
			LoadCambioPass('aplicacion/BusinessLogic/ConsultaExiste.php','CambioPass');
	}
