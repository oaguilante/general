...
function guardar_ficha(_enviar_mail_tarea,_formulario,_nativa) {
    
   // alert("GUARDAR_FICHA"); 
    
  //PARA QUE NO APRENTEN MULTIPLES VECES GUARDAR Y SE DUPLIQUEN LOS REGISTROS
  if(_nativa==true){
    $('#boton_guardar').button("disable");
  }

  _ventana = document.forms[_formulario].var_ventana.value;
  _IdSucursaltmp = document.forms[_formulario].var_id_sucursal.value;
  ...
  if(_IdSucursaltmp==''){
    ...
    _IdSucursal=_IdSucursaltmp;
  }
 
  //alert("BOTON_GUARDAR: "+_enviar_mail_tarea+"-"+_formulario+"-"+_nativa);
  ...
  if(_formulario!="")
    _entidades_ventana_array = [_formulario];
  else
    _entidades_ventana_array = string_to_array(document.forms["ventanaForm"].nombre_forms_string.value,"^");
   
  _cant_entidades_ventana = _entidades_ventana_array.length;
 
  _m = 0;
  _validar_entidad = true;
  _cadena_valores = "";
  
  //PARA CADA UNA DE LAS ENTIDADES DE LA VENTANA QUE SE DEBA GUARDAR
  while(_m<_cant_entidades_ventana && _validar_entidad==true) {

    _formulario = _entidades_ventana_array[_m];
	_validar_entidad = camposFichaObligatorios(_formulario);
    _cadena_valores = _cadena_valores + crearCadenaValoresFicha(_formulario,"");
	
	//CASO PARTICULAR MULTICAJA: VALIDAR QUE LA DIRECCION HAYA SIDO GEOREFERENCIADA
	if(_validar_entidad==true && _ventana=="informe_potencial_cliente"){
	  if(document.forms["potencial_clienteForm"].elements["lat_direccion"].value==""){
	    _validar_entidad = false;
	...
	//PARA QUITAR LAS VALIDACIONES EN DESARROLLO
	//_validar_entidad = true;
	
	//alert(_validar_entidad);
	
    if(_validar_entidad==false) {
      //<JasobNoObfsStr>
	  
	  //alert(georeferenciado);
	  
        //MOSTRAR EL MENSAJE DEVUELTO POR EL SERVER (INCLUSO CASO PARTICULAR GEOREF MULTICAJA)
		if(georeferenciado==true){
	      _mensaje = "Debe ingresar todos los campos obligatorios.";
		...
		else if(georeferenciado==false){
		  _mensaje = "Debe georeferenciar la dirección para poder guardar.";
		}
		
		if(_nativa==true)
		  alert(_mensaje);
		else if(_nativa==false)
	      alert_pro(_mensaje);
		 //</JasobNoObfsStr>
    }
  ...
  if(_validar_entidad==true) {
    ...
    if(document.forms[_formulario].var_ventana.value=="boleta") {
      ...
      if(parseInt(document.forms["formaPagoForm"].efectivo.value)>0 || parseInt(document.forms["formaPagoForm"].cheque.value)>0 || parseInt(document.forms["formaPagoForm"].debito.value)>0 || parseInt(document.forms["formaPagoForm"].credito.value)>0 || parseInt(document.forms["formaPagoForm"].nota_credito.value)>0){
        document.forms[_formulario].var_accion.value='boton_guardar';
	...
	else if(document.forms[_formulario].var_ventana.value=="ficha_fecha_termino_tarea"){
	  
	  _fecha_inicio = document.forms["fecha_termino_tareaForm"].elements["fecha_inicio"].value;
	  _fecha_termino = document.forms["fecha_termino_tareaForm"].elements["fecha_termino"].value;
		
	  if(_fecha_termino.substr(4,1)=="-");
	    _fecha_termino = _fecha_termino.substr(8,2)+"/"+_fecha_termino.substr(5,2)+"/"+_fecha_termino.substr(0,4);
		
	  //alert(_fecha_inicio+"-"+_fecha_termino);
		
	  if(string_a_fecha_mysql(_fecha_inicio)<=string_a_fecha_mysql(_fecha_termino)){
	  
	    _tipo_fecha_termino = document.forms["fecha_termino_tareaForm"].elements["tipo_fecha_termino"].value;
	    _mensaje = "Tarea se cerrará con fecha: "+_fecha_termino;
	  
	    //SI ES UNA FECHA DE CIERRE REAL O DE CIERRE ESTIMADA, PEDIR CONFIRMAR EL CIERRE DE LA TAREA
	    if(_tipo_fecha_termino=="1" || _tipo_fecha_termino=="4"){
	      //showPopup('popup',URLDecode(_mensaje),'CONFIRM','',_formulario);
		  
		  var _r=confirm(URLDecode(_mensaje));
		  
		  if(_r==true){
		    confirmar(_formulario,'');
		  }
		 
		  
	    }
	    //SI ES UNA FECHA COMPROMETIDA, SOLO GUARDAR
	    else if(_tipo_fecha_termino=="2"){
	      //CREAR EL STRING CON LOS DATOS A SER ENVIADOS AL ARCHIVO PHP
          document.forms[_formulario].var_accion.value='boton_guardar';
          _cadena_valores = _cadena_valores + crearCadenaValoresAdicionalesFicha(_formulario,"");
          _cadena_valores = _cadena_valores + "&enviar_mail_tarea=" + _enviar_mail_tarea;
          loadurl_xml('PageLogicPHP.php',_cadena_valores,triggered_xml);
	    }
    ...
    //2.3.-PARA TODOS LOS CASOS (EXCEPTO BOLETA DE VENTA Y FECHA DE TERMINO COPEC)
	  ...
	  //DETERMINAR EL TIPO DE COMUNICACION QUE SE DEBE USAR
	  //if(_ventana=='informe_punto_venta' || _ventana=='informe_punto_venta_spm'){
	  
	  //SI ES MOVIL ESTAN DISPONIBLES TODOS LOS TIPOS DE COMUNICACION
	  if(_nativa==true){
	    var _TipoIngreso = '';
		//PRIMERO, EVALUA EL TIPO DE COMUNICACION ASIGNADO A LA SUCURSAL
		if(document.forms[_formulario].var_tipoComunicacion.value==0 || document.forms[_formulario].var_tipoComunicacion.value==''){
		  //SI LA SUCURSAL NO TIENE TIPO DE COMUNICACION ASIGNADO, SE USA EL DEL USUARIO
		  _TipoIngreso = document.forms[_formulario].var_on_of.value;
		}
		else{
		  _TipoIngreso = document.forms[_formulario].var_tipoComunicacion.value;	
		}
	  }
	  //SI ES DESKTOP SOLO ESTA DISPONIBLE ONLINE
	  else{
	    _TipoIngreso = 1;
	  }
	
	  //PARA DAR UN TIPO DE INSERCION DETERMINADA
	  //_TipoIngreso = 2; 
		
	  //DETERMINAR FECHA/HORA A LA QUE SE REALIZA EL ENVIO DEL REGISTRO
	  var _f = new Date();
	  _Fecha = _f.getFullYear()+"-"+(_f.getMonth() +1)+"-"+_f.getDate()+" "+_f.getHours()+":"+_f.getMinutes()+":"+_f.getSeconds();
	  
	  //alert("TIPO DE COMUNICACION: "+_TipoIngreso);
	  
	  //1: COMUNICACION ONLINE
	  if(_TipoIngreso==1){
		...
		if(_nativa==true){
	      ...
	      _mensaje = "Guardando...";
	      showPopup('popup',URLDecode(_mensaje),'SAVE','',_formulario);
        } 
		
		_cadena_valores = _cadena_valores+'&id_formato_envio=1&fecha_hora_envia='+_Fecha;
		loadurl_xml('PageLogicPHP.php',_cadena_valores,triggered_xml);
	  }
	  //2: COMUNICACION OFFLINE
	  else if(_TipoIngreso==2){
		_cadena_valores = _cadena_valores+'&id_formato_envio=2&fecha_hora_envia='+_Fecha;
		IngresoDataOfline(_cadena_valores,'IPDV','IPDV',_ventana,_IdSucursal);
	  }
	  //3: COMUNICACION ONLINE/OFFLINE
	  else if(_TipoIngreso==3){
	  var _token_sesion = window.sessionStorage.getItem("token_sesion");
		if(!window.navigator.onLine){
		  //alert("1");
		  _cadena_valores = _cadena_valores+'&id_formato_envio=2&fecha_hora_envia='+_Fecha+'&token_sesion='+_token_sesion;
		  IngresoDataOfline(_cadena_valores,'IPDV','IPDV',_ventana,_IdSucursal);
		}
		else{
		  //alert(token_sesion);
          ...
          $('#boton_guardar').button("disable");
		  mostrar_loader("Guardando...");
		  _cadena_valores = _cadena_valores+'&id_formato_envio=1&fecha_hora_envia='+_Fecha+'&token_sesion='+_token_sesion;
		  loadurl_xml('PageLogicPHP.php',_cadena_valores,triggered_xml);
          
		}
	  } 
	  //4: COMUNICACION OFFLINE/ONLINE
	  else if(_TipoIngreso==4){
	    _cadena_valores = _cadena_valores+'&id_formato_envio=2&fecha_hora_envia='+_Fecha;
		IngresoDataOfline(_cadena_valores,'IPDV','IPDV',_ventana,_IdSucursal);
	  } 
...
function camposFichaObligatorios(_formulario) {

  _var_ventana = document.forms[_formulario].elements["var_ventana"].value;

  if(_formulario=="") {
    alert("Falta parametrizar el formulario en la BD");
    _fichaConObligatorios = false;
  }
  else {
  
    _fichaConObligatorios = "";
    ...
    if(_metadata_obligatoria!="") {
      ...
      //alert(_tipo_estructura);
      if(_tipo_estructura=="ficha" || _tipo_estructura=="2" || _tipo_estructura=="datos" || _tipo_estructura=="1"){
     
	    //alert(_cant_metadata);
	 
        for(_i=0;_i<_cant_metadata;_i++){
		
		  //alert(_tipo[_i]);
          if(_tipo[_i]=='INPUT' || _tipo[_i]=='TEXTAREA' || _tipo[_i]=='PASSWORD' || _tipo[_i]=='HIDDEN'){
            _expresion = _expresion + "document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].value!='' & ";
		  }
		  else if(_tipo[_i]=='CHECKBOX'){
            
			_nativa = document.forms["ventanaForm"].elements["nativa"].value;
			
			if(_nativa==false)
			  _expresion = _expresion + "document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].value!='' & ";
			else if(_nativa==true)
			  _expresion = _expresion + "document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].options[document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].selectedIndex].text!='' & ";
			
		  }
          else if(_tipo[_i]=='SELECT'){
			//SI HAY BLANCO_EN_SELECT
			if(document.forms[_formulario].elements[_metadata_obligatoria[_i]].selectedIndex==-1){
			  _expresion = _expresion + "document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].selectedIndex!=-1 & ";
			}
			else{
			  _expresion = _expresion + "document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].options[document.forms[_formulario].elements[_metadata_obligatoria["+_i+"]].selectedIndex].text!='' & ";
			  //document.forms[_formulario].elements[_metadata_obligatoria[_i]].style.backgroundColor = '#f3f3f3';
			}
		  }
		  //VALIDACION LISTAS AZULES (SUBFORMULARIOS)
		  
		  
          else if(_tipo[_i]=='LISTA'){
		
		    //alert(_metadata_obligatoria[_i]);
		 
		  //SOLO FALTA QUE ESTOS PARAMETROS SALGAN DE LA BD!!!
		 
		    //PRODUCTOS INFORME DE TOMA DE PRECIOS
			if(_var_ventana=='informe_toma_precios' && _metadata_obligatoria[_i]=='levantamiento_productos'){
			  var _tipo_subformulario = ["CHECKBOX","CHECKBOX","INPUT"];
			  var _metadata_obligatoria_subformulario = ["id_hay_stock","id_tiene_fleje","precio_sala"];
			}
			//PROMOCIONES ACTIVAS INFORME DE PUNTO DE VENTA MAY
			else if(_var_ventana=='informe_punto_venta' && _metadata_obligatoria[_i]=='promociones_activas'){
			  var _tipo_subformulario = ["SELECT"];
			  var _metadata_obligatoria_subformulario = ["id_estado_promocion","stock_promocion","stock_elementos","carteleria_promociones","exhibicion_adicional"];
			}
			//PROMOCIONES FUTURAS INFORME DE PUNTO DE VENTA MAY
			else if(_var_ventana=='informe_punto_venta' && _metadata_obligatoria[_i]=='promociones_futuras'){
			  var _tipo_subformulario = ["SELECT","SELECT","SELECT","SELECT"];
			  var _metadata_obligatoria_subformulario = ["negociado","espacio","materiales"];
			}
			//PROMOCIONES ACTIVAS INFORME DE PUNTO DE VENTA SPM 
			else if(_var_ventana=='informe_punto_venta_spm' && _metadata_obligatoria[_i]=='promociones_activas_spm'){
			  var _tipo_subformulario = ["SELECT","SELECT","SELECT","SELECT"];
			  var _metadata_obligatoria_subformulario = ["precio_corresponde","id_producto_suficiente","carteleria_promociones","exhibicion_adicional"];
			}
			//PROMOCIONES ACTIVAS INFORME DE PUNTO DE VENTA  COB
			else if(_var_ventana=='informe_spv_cobertura' && _metadata_obligatoria[_i]=='promociones_activas_cob'){
			  var _tipo_subformulario = ["SELECT","SELECT","SELECT","SELECT","SELECT"];
			  var _metadata_obligatoria_subformulario = ["id_sala_informada","precio_corresponde","id_producto_suficiente","carteleria_promociones","exhibicion_adicional"];
			}
			//PROMOCIONES FUTURAS INFORME DE PUNTO DE VENTA SPM Y COB
			else if((_var_ventana=='informe_punto_venta_spm' && _metadata_obligatoria[_i]=='promociones_futuras_spm') || (_var_ventana=='informe_spv_cobertura' && _metadata_obligatoria[_i]=='promociones_futuras_cob')){
			  var _tipo_subformulario = ["SELECT","SELECT"];
			  var _metadata_obligatoria_subformulario = ["id_sala_informada_futura","id_producto_suficiente_futura"];
			}
			//EVALUACION MERCADERISTA IPdV
			else if((_var_ventana=='informe_punto_venta' || _var_ventana=='informe_punto_venta_spm') && _metadata_obligatoria[_i]=='evaluacion2'){
			  var _tipo_subformulario = ["SELECT","SELECT","SELECT","SELECT","SELECT","SELECT","SELECT"];
			  var _metadata_obligatoria_subformulario = ["cumple_horario","uniforme","material_limpieza","material_trabajo","limpieza_gondola","limpieza_producto","exhibicion"];
			}
			//VALIDAR QUE HAYAN ELEMENTOS EN LA LISTA (CHEQUEAR EL PRIMER CAMPO DEL SUBFORMULARIO) Y POR ENDE SE HAYA DIBUJADO AL MENOS UN SUBFORMULARIO
			if(isset(document.forms[_formulario].elements[_metadata_obligatoria_subformulario[0]+"0"])==true){
			
			  var _cant_elementos_lista =  $('#'+_metadata_obligatoria[_i]).children().size(); 
			
			  //VALIDAR CADA SUBFORMULARIO DE LA LISTA
			  for(_k=0;_k<_cant_elementos_lista;_k++){
			  
			    //VALIDAR CADA CAMPO DEL SUBFORMULARIO 
			    for(_j=0;_j<_metadata_obligatoria_subformulario.length;_j++){
			      if(_tipo_subformulario[_j]=='INPUT'){
                    _expresion = _expresion + "document.forms[_formulario].elements['"+_metadata_obligatoria_subformulario[_j]+_k+"'].value!='' & ";
		          }
			      else if(_tipo_subformulario[_j]=='CHECKBOX'){
			        _expresion = _expresion + "document.forms[_formulario].elements['"+_metadata_obligatoria_subformulario[_j]+_k+"'].options[document.forms[_formulario].elements['"+_metadata_obligatoria_subformulario[_j]+_k+"'].selectedIndex].text!='' & ";
		          }
			      else if(_tipo_subformulario[_j]=='SELECT'){
			        //SI HAY BLANCO_EN_SELECT
					//SI EL CAMPO NO EXISTE, NO DEBIESE PERMITIR GUARDAR!!!
			        if(document.forms[_formulario].elements[_metadata_obligatoria_subformulario[_j]+_k].selectedIndex==-1){
			          _expresion = _expresion + "document.forms[_formulario].elements['"+_metadata_obligatoria_subformulario[_j]+_k+"'].selectedIndex!=-1 & ";
			        }
		...
		else if(_tipo[_i]=='LISTA_BASICA_CHECK'){
		//alert(_metadata_obligatoria[_i])
				if(_metadata_obligatoria[_i]=="check_out2" || _metadata_obligatoria[_i]=="quiebre_bestseller" || _metadata_obligatoria[_i]=="quiebre" ){
					var cantidad = document.getElementsByName('gps'+_metadata_obligatoria[_i]);
						for (x=0;x<cantidad.length;x++)
						{
							var namehdd = _metadata_obligatoria[_i]+x;
							if(document.getElementById('SioNo'+namehdd).value=='0'){
							
								_expresion = _expresion + "document.forms[_formulario].elements['SioNo"+namehdd+"'].value!='0' & ";
								//alert(_expresion)
							}
		...
		//alert(_expresion)
        _expresion = _expresion.substr(0,_expresion.length-2);
		//alert(eval(_expresion))
	   if(eval(_expresion)){
		 _fichaConObligatorios = true;
		}
        else{
          _fichaConObligatorios = false;
        }
		
		//alert(_fichaConObligatorios);
		
      }
      else if(_tipo_estructura=="grid" || _tipo_estructura=="4"){

	    //alert("VALIDAR_GRID");
        if(validarGrid(_formulario)) 
          _fichaConObligatorios = true;
        else
          _fichaConObligatorios = false;
	    ...
	    _fichaConObligatorios = false;
	    alert("ERROR DE PARAMETRIZACION\nTipo de estructura no reconocido: "+_tipo_estructura);
		die();
      }
	...
	//SI NO HAY CAMPOS OBLIGATORIOS DEFINIDOS PARA LA ENTIDAD...
	  ...
	  //...Y ADEMAS ES UN GRID
	  if(_tipo_estructura=="grid" || _tipo_estructura=="4")  {
	  
	    _id_grid = document.forms[_formulario].var_entidad.value;
	    _cant_filas = document.getElementById(_id_grid).rows.length - 1;
...
//alert("VALIDA: "+_fichaConObligatorios+"-"+_tipo_estructura);

return _fichaConObligatorios;
...
function crearCadenaValoresFicha(_formulario,_id_fila) {
  //CARGAR PARAMETROS REQUERIDOS
  _nativa = document.forms["ventanaForm"].elements["nativa"].value;
  _entidad = document.forms[_formulario].elements["var_entidad"].value;
  _metadata = document.forms[_formulario].elements["var_metadata"].value.split("^");
  _tipo = document.forms[_formulario].elements["var_tipo"].value.split("^");
  //_formato_visual = document.forms[_formulario].elements["var_formato_visual"].value.split("^");
  _tipo_entidad = document.forms[_formulario].elements["var_tipo_estructura"].value;
  //_var_ventana = document.forms[_formulario].elements["var_ventana"].value;
  
  
 
  
  //NO DEBE CONSIDERAR tarea_actual (por eso se quita uno)
  _cant_metadata = _metadata.length;
  //_cant_metadata = _metadata.length -1;
  ...
  _cadena_valores = "";
  _grid_completo = "";
  _hacia_bd = document.forms[_formulario].elements["var_hacia_bd"].value.split("^");
  
  //alert(_hacia_bd);
  
  //SI NO ES UN GRID, NO COMPROBAR SUS FILAS
  if(_id_fila==""){
    _grid_completo = true;
  }
  ...
  if(_tipo_entidad=="grid" && _grid_completo==true) { 
    _cant_registros_por_entidad = parseInt(document.forms[_formulario].elements["var_filas"].value)+1;
  }
  else {
    _cant_registros_por_entidad = 1;
  }

  //alert(_cant_registros_por_entidad);
  //POR CADA UNO DE LOS REGISTROS DE LA ENTIDAD
  for(_a=0;_a<_cant_registros_por_entidad;_a++){

    for(_i=0;_i<_cant_metadata;_i++){
	
	  if(_hacia_bd[_i]=="s"){
	
        if(_tipo[_i]=='input' || _tipo[_i]=='hidden' || _tipo[_i]=='textarea' || _tipo[_i]=='password') {
          //CUANDO ESTOY GUARDANDO Y HAY UN GRID AGREGAR TODOS LOS VALORES DEL GRID
          if(_tipo_entidad=="grid" && _grid_completo==true){
            _valor = document.getElementById(_entidad).rows[_a+1].cells[_i].children[0].value;
		  }
          //CUANDO ESTOY BUSCANDO EN UNA FICHA O UN GRID, USAR SOLO EL ELEMENTO CORRESPONDIENTE (1 FILA)
          else{
           // alert(_metadata);
            _valor = document.forms[_formulario].elements[_metadata[_i]+_id_fila].value;  
		  
		    //CONVERTIR FORMATO DE FECHAS PARA CUANDO SE CAPTURAN USANDO EL WIDGET EN EL MOVIL
		    if(_valor.substr(4,1)=="-" && _valor.substr(7,1)=="-"){  
		      _valor = _valor.substr(8,2)+"/"+_valor.substr(5,2)+"/"+_valor.substr(0,4);
		    }
          ...
          if(_tipo_entidad=="grid" && _grid_completo==true) {
            ...
            if(document.getElementById(_entidad).rows[_a+1].cells[_i].children[0].value==99)
              _valor = "";
            else
              _valor = document.getElementById(_entidad).rows[_a+1].cells[_i].children[0].value;
          }
          //CUANDO ESTOY BUSCANDO EN UNA FICHA O UN GRID, USAR SOLO EL ELEMENTO CORRESPONDIENTE (1 FILA)
            ...
            if(document.forms[_formulario].elements[_metadata[_i]+_id_fila].value==99)
              _valor = "";
            else
              _valor = document.forms[_formulario].elements[_metadata[_i]+_id_fila].value;
          }
	      ...
	      if(_nativa==false){
            if(document.forms[_formulario].elements[_metadata[_i]+_id_fila].checked==1)
              _valor = "1";
            else if(document.forms[_formulario].elements[_metadata[_i]+_id_fila].checked==0)
              _valor = "0";
		  }
		  else if(_nativa==true){
		    //EL CHECKBOX DE JQUERY EN REALIDAD ES UN SELECT
		    _valor = document.forms[_formulario].elements[_metadata[_i]].value;
		  } 
		  
        }
        else if(_tipo[_i]=='input_list' || _tipo[_i]=='select_list') {
       
          _list = _metadata[_i];
          _cant_options = document.forms[_formulario].elements[_list].length;

          _valor = "";

          for(_m=0;_m<_cant_options;_m++) {
		    //TOMAR EL VALOR DE LA OPCION Y NO EL TEXTO VISIBLE
            _valor = _valor + document.forms[_formulario].elements[_list].options[_m].value + ',';
			
          }

          //QUITAR EL ULTIMO SIMBOLO DE SEPARACION
          if(_valor.charAt(_valor.length-1)==",")
            _valor = _valor.substring(0, _valor.length-1);

		
		  //ORDENAR LOS ID_TAREA DE MENOR A MAYOR (SOLO AL GUARDAR)
		  if(_valor!="") {
		    _valor_array = string_to_array(_valor,",");
		    _valor_array.sort(function(_a,_b){return _a - _b});
		    _valor = array_to_string(_valor_array,",");
          }
		  
        }
	    //LISTAS AZULES (SUBFORMULARIOS)
	    else if(_tipo[_i]=='lista'){
	 
	      //ARCOR: ESTO SE DEBE GENERALIZAR!! 
	
		  if(_metadata[_i]=="levantamiento_productos"){
		    _z = 0;
			_valor="";
			
			//MIENTRAS EL ID TENGA UN VALOR...
			while(isset(document.forms[_formulario].elements["id_producto"+_z+""])==true){
			  _valor = _valor + document.forms[_formulario].elements["id_producto"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_hay_stock"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_tiene_fleje"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["precio_sala"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_exhibicion_adicional"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_pedido_sala"+_z+""].value + "#"; 
			  _z = _z + 1;
			}
		  }
	
	      //INFORME PUNTO DE VENTA MAY
		  if(_metadata[_i]=="promociones_activas"){
		    var _z = 0;
		    var _k = 0;
		    var _valor = "";
		    var _metadata_subformulario = ["id_accion_activa","id_estado_promocion","id_detalle_promocion","stock_promocion","carteleria_promociones","observaciones_promocion","exhibicion_adicional","stock_elementos"];
            var _cant_metadata_subformulario = _metadata_subformulario.length;
		 
		    //SERIALIZAR TODOS LOS CAMPOS DE CADA SUBFORMULARIO (z CORRESPONDE AL CORRELATIVO DEL SUBFORMULARIO)
		    while(isset(document.forms[_formulario].elements[_metadata_subformulario[_k]+_z+""])==true){
		  
		      for(_k=0;_k<_cant_metadata_subformulario;_k++){
		        if(_k==0)
			      _valor = _valor + document.forms[_formulario].elements[_metadata_subformulario[_k]+_z+""].value;
			    else 
               //alert(_valor);
			      _valor = _valor + "," + document.forms[_formulario].elements[_metadata_subformulario[_k]+_z+""].value;
		  ...
		  //INFORME PUNTO DE VENTA SPM Y COB (PROMOCIONES ACTIVAS)
		  if(_metadata[_i]=="promociones_activas_spm"){
			_z = 0;
			_valor="";
			while(isset(document.forms[_formulario].elements["id_accion_activa"+_z+""])==true){
			  _valor = _valor + document.forms[_formulario].elements["id_accion_activa"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_producto_suficiente"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["carteleria_promociones"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["exhibicion_adicional"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["precio_corresponde"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["precio_sala"+_z+""].value + "#";  
			  _z = _z + 1;
			}
		  }
			
			if(_metadata[_i]=="promociones_activas_cob"){
			_z = 0;
			_valor="";
			while(isset(document.forms[_formulario].elements["id_accion_activa"+_z+""])==true){
			  _valor = _valor + document.forms[_formulario].elements["id_accion_activa"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_sala_informada"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_producto_suficiente"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["carteleria_promociones"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["exhibicion_adicional"+_z+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["precio_corresponde"+_z+""].value;
		  ...
		  //INFORME PUNTO DE VENTA MAY (PROMOCIONES FUTURAS)
		  if(_metadata[_i]=="promociones_futuras"){
		    _r = 0;
			_valor="";
				
			while(isset(document.forms[_formulario].elements["negociado"+_r+""])==true){
			  _valor = _valor + document.forms[_formulario].elements["id_accion_futura"+_r+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["negociado"+_r+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["espacio"+_r+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["materiales"+_r+""].value+ "#";  
			  _r = _r + 1;
			}
		  }
		
		  //INFORME PUNTO DE VENTA SPM Y COB (PROMOCIONES FUTURAS)
		  if(_metadata[_i]=="promociones_futuras_spm" || _metadata[_i]=="promociones_futuras_cob"){
		    _r = 0;
			_valor="";
			
			while(isset(document.forms[_formulario].elements["id_accion_futura"+_r+""])==true){
			  _valor = _valor + document.forms[_formulario].elements["id_accion_futura"+_r+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_sala_informada_futura"+_r+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["id_producto_suficiente_futura"+_r+""].value+ "#";  
			  _r = _r + 1;
		    }	
		  }

		  //EVALUACION AL MERCADERISTA
		  if(_metadata[_i]=="evaluacion2"){
			_j = 0;
			_valor="";
			while(isset(document.forms[_formulario].elements["id_mercaderista"+_j+""])==true){
			  
			  _valor = _valor + document.forms[_formulario].elements["id_mercaderista"+_j+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["cumple_horario"+_j+""].value
			  _valor = _valor + "," + document.forms[_formulario].elements["uniforme"+_j+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["material_limpieza"+_j+""].value;
			  _valor = _valor + "," + document.forms[_formulario].elements["material_trabajo"+_j+""].value+",";
				
			  //FIFO EVALUACION MERCADERISTA
			  var _fifo=document.getElementsByName('gpsfifo0');
			  if(_fifo=='[object NodeList]'){
			    for(_fi=0;_fi<_fifo.length;_fi++){
				  _valor = _valor + document.getElementById("IDSioNofifo0"+_fi).value;
				  _valor = _valor + "@" + document.getElementById("SioNofifo0"+_fi).value + "*";
				}	
				_valor = _valor + ",";
			  }
			  
			  //FLEJES EVALUACION MERCADERISTA
			  /*var _promo=document.getElementsByName('gpsflejes_promocion0');
					
			  if(_promo=='[object NodeList]'){
				for(_pro=0;_pro<_promo.length;_pro++){
				  _valor = _valor + document.getElementById("IDSioNoflejes_promocion0"+_pro).value;
			      _valor = _valor + "@" + document.getElementById("SioNoflejes_promocion0"+_pro).value + "*";
				}	
				//_valor = _valor + "@@@,";		
			  }*/
			  
			  //_valor = _valor + "," + document.forms[_formulario].elements["producto_flejes"+_j+""].value; 
			  
			  _valor = _valor + "," + document.forms[_formulario].elements["limpieza_gondola"+_j+""].value;  
			  _valor = _valor + "," + document.forms[_formulario].elements["limpieza_producto"+_j+""].value;  
			  _valor = _valor + "," + document.forms[_formulario].elements["exhibicion"+_j+""].value+",";
			  //QUIEBRES EVALUACION MERCADERISTA
			  var _quieb=document.getElementsByName('gpsquiebre0');
			  
			  if(_quieb=='[object NodeList]'){
			    for(_qui=0;_qui<_quieb.length;_qui++){
				  _valor = _valor + document.getElementById("IDSioNoquiebre0"+_qui).value;
				  _valor = _valor + "@" + document.getElementById("SioNoquiebre0"+_qui).value + "*";
				}
				//_valor = _valor + "@@@,";
			  }	
			  _valor = _valor + "," + document.forms[_formulario].elements["comentario"+_j+""].value+ "#"; 
	    ...
	    //LISTAS BASICA CHECK
	      ...
	      _cant_elementos_lista = document.getElementsByName("gps"+_metadata[_i]).length;
		  _valor = "";
		
		  for(_p=0;_p<_cant_elementos_lista;_p++){
		    _valor = _valor + document.forms[_formulario].elements["IDSioNo"+_metadata[_i]+_p].value;
		    _valor = _valor + "," + document.forms[_formulario].elements["SioNo"+_metadata[_i]+_p].value + "#";
		  }	
        ...
        if(_tipo_entidad=="grid" && _grid_completo==true){ 
          ...
          if(_a==0 && _i==0){
            _cadena_valores = _cadena_valores + "valores_grid=";
          }
		  
          _cadena_valores = _cadena_valores + URLEncode(_valor) + '^';

          if(_i==_cant_metadata-1 && _a!=_cant_registros_por_entidad-1)
            _cadena_valores = _cadena_valores.substr(0,_cadena_valores.length-1) + '#';
          else if(_i==_cant_metadata-1 && _a==_cant_registros_por_entidad-1)
            _cadena_valores = _cadena_valores.substr(0,_cadena_valores.length-1) + '&';

        }
        else{ 
		  _cadena_valores = _cadena_valores + _metadata[_i] + '=' + URLEncode(_valor) + '&';
        }
