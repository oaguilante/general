...
function inicializar_google_maps(mobile,server,id_supervisor,tipo_key_id,key_id,lat,long,filtros){
  ...
  if(ventana=="mapa_sucursales" || ventana=="mapa_sucursales_di"){
	...
	//PHONEGAP
	//loadurl_xml('http://192.168.1.183/angular/aplicacion/BusinessLogic/action_buscar_sucursales.php',cadena_parametros,triggered_mapa_xml);
    ...
    cadena_parametros = "tipo_key_id="+tipo_key_id+"&cod_interno="+key_id;
    loadurl_xml('action_latitud.php',cadena_parametros,triggered_mapa_xml);
  } 
...
function inicializando(lat,long,mensaje,distancia,mobile,server,xml_root,tipo_key_id) {
  ...
  for(var i=0;i<cant_filas;i++){
    ...
    markers.push(marker);
	  ...
	  loadurl_xml('cambio_lat_long.php',cadena_parametros,triggered_mapa_Lat_Long);	
	  }
	  })
	  (marker, i));
	  ...
	  //CREAR EVENTO CLICK DEL MARCADOR
		...
		return function() {
		 ...
		 codigo_local = URLDecode(xml_entidad.getElementsByTagName("codigo_local").item(i).firstChild.data); 
			...
			$.post('http://192.168.1.183/angular/aplicacion/BusinessLogic/consulta_datos_punto.php',
			{
...
function foundLocation(position){ 
  ...
  if(mobile=='false'){
	...
	if(ventana=="lista_sucursales"){filtros='';}else{
